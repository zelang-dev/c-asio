...
static void async_close(uv_handle_t *const handle) {
...
string async_tls_read(async_tls_t *const socket) {
	...
	for (;;) {
		ssize_t x = tls_read(socket->secure, socket->data, max);
		if (x >= 0) {
			socket->buf = buf;
			return buf;
		}

		if (tlserr(tls_poll((uv_stream_t *)socket->stream, (int)x), socket->secure) < 0)
